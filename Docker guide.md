...
#### `pstree -ns <PID>` — показать дерево процессов + namespace info:
```bash
pstree -ns 2480
```
...
<details>
<summary>⚠️ Почему Alpine может ломать Python/Java</summary>

### ⚠️ Важное замечание про `alpine` и `musl`

Alpine использует **`musl libc` вместо стандартной `glibc`** (GNU C Library), на которой написано большинство Linux-приложений.

#### В чём проблема:
- Некоторые бинарные пакеты (особенно в Python/Java) **компилированы под `glibc`**
- `musl` не на 100% совместим с `glibc`
- Это может вызывать ошибки вида:
  ```
...
- Java-приложения с native-библиотеками
- Python с C/C++-расширениями

#### ✅ Что делать:

1. Не использовать Alpine, если у тебя научный стек или нативные зависимости
2. Или установить `glibc` вручную:
    ```Dockerfile
    RUN apk add --no-cache gcompat
    ```
3. Или использовать `python:3.11-slim` вместо `alpine`
...
📌 Хочешь полный контроль — используй **обёрточный shell-скрипт** + `ENTRYPOINT`
...
📌 Эти переменные будут внутри контейнера как обычные `ENV` (можно посмотреть через `printenv` или `echo $APP_ENV`).
...
### ⚠️ Почему `ADD` считается плохой практикой?
